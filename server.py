...
def disconnectWithClient(message, client, client_name):
    ...
    clientName = message[11:].strip()
    if(clientName in clients):
        clients[clientName]["connected_with"] = ""
        clients[client_name]["connected_with"] = ""
        
        otherClientSocket = clients[clientName]["client"]
        
        greetMessage = f"Hello, {clientName} {client_name} disconnected with you!!"
        otherClientSocket.send(greetMessage.encode())
        
        msg = f"You are successfully disconnected with {clientName}"
